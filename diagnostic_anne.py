...
###############################################################################
# Write f05 that for each string in a a parameter, prints that 
...
def f06(string1, string2):
    if len(string1) > len(string2):
        ...
        per = (float(len(string2))/len(string1)) * 100
        print string1 + " is longer than " + string2 + " by " + str(x) + " chars" # i don't know how to use .format.
        print string2 + " has only " + str(per) + " percent the number of chars of " + string1
    else:
        x = len(string2) - len(string1)
        per = (float(len(string1))/len(string2)) * 100
        print string2 + " is longer than " + string1 + " by " + str(x) + " chars"
        print string1 + " has only " + str(per) + " percent the number of chars of " + string2

#i know there must be an easier, more modular/efficient way.
...
def various_solutions():
    ...
    while_ = f07(1)
    for_ = f08(range(1,500))
    list_comprehension = f09(1)
    recursion = f10(1)
...
def f09(x):
    pass
    # [total += x for x < 500 if (x%3 == 0) or (x%5 == 0)]
    # x += 1
    # return total
...
user_input_list = []
def f12():
    user_input = raw_input('>Asking for input\n')
    if user_input != 'done':
        try: 
            float(user_input)
        except:
